    ...
    public double[] exMethod() throws OptimizationException {
        ...
        for (int i = 0; i < bank.getUnions().size(); i++) {
            if (bank.getUnions().get(i).getAgents().size() == 1) {
                /**
                 * Будущая прибыль в коалиции должна быть больше или равна прибыли агента без коалиции минус надбавка
                 */
                tmp[i] = -1;
                tmp[i + countAgent] = 1;
                ...
                if (bank.getUnions().get(i).getAgents().size() == countAgent) {
                    /**
                     * Оптимальная прибыль агентов должна быть равна сумме будущих прибылей агентов
                     */
                    ...
                    /**
                     * Сумма будущих прибылей агентов (по комбинациям) должна быть больше или равна
                     * прибыли этих агентов из оптимального пакета проектов
                     */
        ...
        for (int i = 0; i < countAgent; i++) {
            /**
             * Записываем условия для надбавок
             */
            tmp[i] = 1;
